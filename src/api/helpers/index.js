...
export const getRequestUrl = (param) => {
  ...
  const dateFrom = prevFilterState.range && prevFilterState.range.from ? prevFilterState.range.from : moment(new Date()).format('YYYY-MM-DDT00:00:00');
  const dateTo = prevFilterState.range && prevFilterState.range.to ? prevFilterState.range.from : false;
